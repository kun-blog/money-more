...
class CloudDBZoneWrapper {
    ...
    async add(payload) {
        ...
        try {
            // 构建DB实例
            const bankCard = clouddb.CloudDBZoneGenericObject.build('bankCard')
            bankCard.addFieldValue("uuid", payload.uuid)
            bankCard.addFieldValue("bank", payload.bank)
            bankCard.addFieldValue("name", payload.name)
            bankCard.addFieldValue("cardNumber", payload.cardNumber)
            bankCard.addFieldValue("expiredAt", payload.expiredAt)
            bankCard.addFieldValue("ccv", payload.ccv)

            return mCloudDBZone.executeUpsert(bankCard)
        } catch (error) {
    ...
    async query (payload) {
        ...
        try { // 构建DB实例
            ...
            console.log(JSON.stringify(resp), '调试——————————')
